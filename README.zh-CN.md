...
只有手动库存商品才显示订单 CDK 和联系客服的交接提示。自动订单等待时只展示进度，
不要求客户发送 GM 订单号；已存在上游采购记录的订单，即使 SKU 后来改成人工，仍按
原自动流程展示。交付后自动展开实际交付原文，保留多行说明，并将原文中的 HTTP(S)
使用地址/教程变成可点击链接；复制的是交付原文，不是 GM 订单号。上游付款元数据
（包括可为空的 `url`）不是充值入口，不能拿来替代使用说明。原文没有使用网址时，可将已核验的上游商品说明入口配置在 `policy_json.supplierUsageGuide`（provider、origin、skuId、url）。必须与该单供应商和SKU身份匹配；新订单付款时固化入口，老订单仅使用身份匹配的补充入口。入口单独展示，不改动复制的交付原文。没有返回或核验的网址就不编造，也不会展示采购金额等内部字段。

### 在线客服微信入口

在线客服不再统一要求客户发送订单 CDK。“直接添加微信”在当前页面弹出已有的客服
二维码，不跳转到图片网址。关闭弹窗后焦点回到入口，聊天记录和未发送草稿保持不变。

### Claude 下单前自查

Claude 充值商品的全部规格共用“三种不可充值情况、两种自查方法、三张示例图”，
商品购买区提供明显入口；直接结算或混合购物车含 Claude 时，结算页也展示一次。
配图存于本站，点击在站内弹窗放大。不会自动创建订单、发消息或请求第三方 IP 检测。

IP 自查已改为本站 `/ip-check`，`GET /api/ip-check` 在原有 Host 校验后读取当前
Cloudflare 连接，禁止缓存、不需要密钥、不写记录。本地预览与 Bun 部署明确显示
信息不足；有限 ASN/组织名规则仅提示疑似机房，VPN、代理、Tor 与滥用信誉明确为未知。
不提供安全分数或防封保证。页面含中英双语详细说明、本地时区及需用户同意才运行的
Google STUN 检查。检测网页与实际客户端可能使用不同出口；不改价格、交付、订单或
后端购买规则。来源、消融和验证记录见 `src/features/ip-check/NOTES.md`。
...
upgrade-disabled PNG；文字保留完整事项并简明改写。普通网络错误或单独的结算失败
不能直接认定为封禁，应先停止充值并联系客服确认。

方法二补充店主提供的两张正常界面截图（方案列表、Max 订单详情），与原封禁示例对照。
“Claude 网页版”和“升级订阅页面”为可跳转链接。检查不需要在官网付款，截图中的
示例价格不代表本店售价，也不作为充值成功保证。

IP 质检位于方法一、方法二之前，明确要求先检测网络再访问官网。方法一增加店主提供、已打码的正常对话截图，与组织禁用示例对照；保留原有打码，不恢复隐藏内容。

页面拆为两个独立板块：先展示“使用前先做 IP 质检”，再展示“Claude 下单必看”。IP 建议不再嵌入下单与退款风险说明内部。
